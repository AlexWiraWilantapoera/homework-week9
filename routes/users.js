...
router.post("/register", (req, res) => {
  if (!req.body.id || !req.body.email || !req.body.gender || !req.body.password || !req.body.role) {
    res.status(400);
...
router.put("/update/:id", (req, res) => {
  ...
  pool.query(`update users set email = '${req.body.email}' where id = ${req.params.id}`, (error, result) => {
    if (error) {
      throw error;
    }
